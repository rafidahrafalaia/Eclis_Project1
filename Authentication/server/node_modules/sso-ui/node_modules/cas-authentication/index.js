...
CASAuthentication.prototype._login = function(req, res, next) {
    ...
    console.log(req.session.cas_return_to);
	if (this.renew) {
        console.log("test");
		var query = {
        ...
        response.on('end', function() {
            this._validate(body, function(err, user, attributes) {
                if (err) {
                    ...
                    console.log(req.session[ this.session_name ])
                    if (this.session_info) {
                        req.session[ this.session_info ] = attributes || {};
                        console.log(req.session[ this.session_info ]);
                    }
                    res.redirect(req.session.cas_return_to);
                }
            }.bind(this));
